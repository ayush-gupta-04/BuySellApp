        ...
        <div className="flex flex-col shadow-lg bg-white rounded-lg">
            <div className="w-full min-h-48 grid grid-cols-12 gap-1  rounded-t-lg border-b">
               <div className="col-span-3  grid grid-rows-5 px-2 pt-2">
                    <div className="row-span-4  overflow-hidden rounded-lg"><img src = {item.photo} alt="" className="rounded-lg border-2 border-black hover:scale-105 transition-all duration-300"/></div>
            ...
            <div className={` w-full overflow-hidden rounded-b-lg ${show?"h-fit":"h-0"} transition-all duration-500`}>
                {item.requestCount > 0 && 
                    item.Buyer.map((buyer_req) => {
                        return (
                            <Request buyer = {buyer_req} show ={show}></Request>
                        )
                    })
                }
            </div>
...
function Request({buyer,show} : {buyer : Buyer_Req,show : boolean}){
        ...
        <div className="min-h-20 w-full border-b grid grid-cols-3">
            ...
            <div className="fixed z-50 w-1/5 h-fit px-2 py-2 bg-white rounded-md top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 flex flex-col gap-2">
                ...
                <div className="flex gap-2 justify-center">
                    ...
                    <button className="bg-red-500 w-full text-white px-8 py-2 rounded-lg active:scale-95 transition-all duration-150 hover:bg-red-600" onClick={(e) => {confirmReject();e.stopPropagation()}}>{loading?"Loading...":"confirm"}</button>
                </div>
            </div>}
        </div>
    )
