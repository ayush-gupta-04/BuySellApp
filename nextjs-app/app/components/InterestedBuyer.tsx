...
import { deleteRequestAction } from "../action/deleteRequest"
...
export default function BuyInterestedCard({item} : {item : ItemToBuyType}){
    ...
    async function confirmDeleteRequest(){
        setLoading(true);
        const res = await deleteRequestAction(item.buy_id) as BackendResponse;
        setResponse(res);
        setLoading(false);
    }
    return (
        <div className="w-full min-h-48 grid grid-cols-12 gap-1 bg-white rounded-md shadow-lg">
            <div className="col-span-3 mt-2 ml-2 flex flex-col justify-between">
                <div className="h-full bg-black overflow-hidden rounded-md flex items-center"><img src={item.photo} alt="" className="hover:scale-110 transition-all duration-300"/></div>
            ...
            <div className="col-span-7  my-2 flex flex-col gap-1">
                <div className="font-semibold text-lg  px-2 text-gray-700 cursor-pointer hover:text-black" onClick={(e) => {router.push(`/dashboard/${item.item_id}`) ; e.stopPropagation()}}>{item.title}</div>
            ...
            <div className="col-span-2 text-center w-full h-full flex flex-col justify-between  items-center px-2 py-2 hover:bg-red-200 rounded-md transition-all duration-500 active:scale-95 cursor-pointer active:bg-red-300" onClick={(e) => {deleteRequest();e.stopPropagation()}}>
                <div></div>
            ...
            <div className="fixed z-50 w-1/5 h-fit px-2 py-2 bg-white rounded-md top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 flex flex-col gap-2">
                ...
                <div className="flex gap-2 justify-center">
                    ...
                    <button className="bg-red-500 w-full text-white px-8 py-2 rounded-lg active:scale-95 transition-all duration-150 hover:bg-red-600" onClick={(e) => {confirmDeleteRequest();e.stopPropagation()}}>{loading?"Loading...":"confirm"}</button>
                </div>
            </div>}
        </div>
    )
